...
- **Linux is verified, not assumed.** WSL2 Debian is on this machine and shares the host
  ...
  privileged on Linux** (needs `setcap` or a high port), and GPU percentage is AMD-only there
  (sysfs `gpu_busy_percent`; NVIDIA and Intel leave the tag alone rather than reading zero).
