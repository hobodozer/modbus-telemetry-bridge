...
// The scaling table is the point of this tool when you are investigating, and noise when it runs
// as a build gate. --quiet keeps the verdicts and drops the table.
...
{
    if (!ok) failures++;
    if (ok && quiet) return;                 // a failure always prints
...
Say("Read cost, 125-register request (the largest a single FC3 can ask for):");
Say("");
Say("  points   holdLastValue/0      failsafe/2000ms");
Say("  ------   ------------------   ------------------");
...
foreach (var pointCount in new[] { 32, 125, 317, 1000 })
{
    ...
    Say($"  {pointCount,6}   {row[0],8:F3} ms/read   {row[1],8:F3} ms/read");
}
...
Say($"  map grew {sizeRatio:F0}x, read cost grew {growth:F1}x");
